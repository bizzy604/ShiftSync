"""
MODULE: /apps/api/app/modules/users/router.py

FUNCTION:
    Exposes users HTTP endpoints through a modular route layer.
...
IMPORTANCE:
    This route layer preserves existing `/users` API behavior while domain scope and
    validation rules are centralized in users services and shared dependencies.
"""
...
from app.shared.dependencies import CurrentUser, ensure_self_or_admin, get_current_user, require_roles
from app.core.database import prisma
from app.core.security import hash_password
from app.modules.users.exceptions import UserAccessDeniedError, UserInvalidClockTimeError
from app.modules.users.service import (
    assert_user_visible_to_actor as assert_user_visible_to_actor_rule,
    ensure_clock as ensure_clock_value,
...
def _ensure_clock(clock_time: str | None) -> None:
    try:
        ensure_clock_value(clock_time)
    except UserInvalidClockTimeError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Invalid clock time: {exc.clock_time}",
        ) from exc


async def _assert_user_visible_to_actor(actor: CurrentUser, target_user_id: str) -> None:
    try:
        await assert_user_visible_to_actor_rule(actor, target_user_id)
