"""
MODULE: /apps/api/app/modules/swaps/service.py

FUNCTION:
    Implements swaps/drop domain workflows, state transitions, and notification fan-out.
...
IMPORTANCE:
    This service centralizes all swap/drop state-machine rules so approvals, declines,
    expirations, and notifications remain consistent across API routes and worker jobs.
"""
...
async def expire_due_drop_requests_for_worker(
    *,
    ws_manager: object | None,
    now: datetime | None = None,
) -> int:
    """Expire due drop requests for the background worker orchestration loop.

    Args:
        ws_manager: Optional websocket manager used for realtime status fan-out.
        now: Optional clock override for deterministic execution in tests.

    Returns:
        Count of drop requests expired during this run.
    """
...
def to_resp(row: object) -> SwapRequestResponse:
    ...
    if requester_assignment:
        ...
        if shift:
            ...
            shift_time = f"{start_local[11:16]} â€“ {end_local[11:16]}"
