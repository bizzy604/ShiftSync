...
async def _location_or_404(location_id: str) -> object:
...
async def overtime_dashboard(
    ...
    week_assignments = await prisma.shiftassignment.find_many(
    ...
    all_assignments = await prisma.shiftassignment.find_many(
...
async def fairness_report(
    ...
    shifts = await prisma.shift.find_many(
        where={
            "location_id": location_id,
            "shift_date": {
                "gte": _date_as_datetime(start_date),
                "lte": _date_as_datetime(end_date),
            },
        },
        order={"start_utc": "asc"},
    )
...
async def hours_distribution(
    ...
    await _location_or_404(location_id)
    if current_user.role == "manager":
        ensure_manager_location_access(current_user, location_id)
...
@router.get("/on-duty", response_model=OnDutyResponse)
async def on_duty(
    ...
    if current_user.role == "admin":
        if location_id:
            ...
            locations = await prisma.location.find_many(where={"is_active": True})
    ...
    active_shifts = await prisma.shift.find_many(
        where={
    ...
    active_assignments = []
    if shift_ids:
        active_assignments = await prisma.shiftassignment.find_many(
            where={"shift_id": {"in": shift_ids}, "status": "assigned"},
            include={
                "user": True,
    ...
    for loc_id in scoped_location_ids:
        ...
        location_obj = shifts_here[0].location if shifts_here else await prisma.location.find_unique(where={"id": loc_id})
